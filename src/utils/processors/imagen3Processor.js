...
export async function processImagen3(interaction, data) {
  const emoji = QUEUE_EMOJIS[QUEUE_TYPES.IMAGEN3];
  const typeName = QUEUE_TYPE_NAMES[QUEUE_TYPES.IMAGEN3];

  try {
    await interaction.editReply(`${emoji} Processing your ${typeName}...`);

    const { generateImagen3 } = await import("../../google/imagenHandler.js");
    const files = await generateImagen3(data.prompt, data.aspectRatio);
